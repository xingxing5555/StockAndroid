...
public class MLineView extends StockLineView {
    ...
    public final static int mline_biankuang = 0xffDDDDDD;//���Ʊ߿�ɫֵ(��ɫ)
//    public final static int mline_biankuang = Color.BLACK;//���Ʊ߿�ɫֵ(��ɫ)
...
//        volumeStartY = endY + FontHeight + 4;
        volumeStartY = endY + FontHeight + 4;
        volumeEndY = screenH - (FontHeight / 2 + 4);
...
//            System.out.println(i+" "+curVolume);
            ...
            if (mlineType == ONE_DAY_MINUTES_LINE) {
                smmaCurve.Amount[i] = minDataList.get(i).get(0) * curVolume + smmaCurve.Amount[i
                        - 1];
            } else {
                if (i % 49 == 0) {
                    smmaCurve.Amount[i] = minDataList.get(i).get(0) * curVolume;
                } else {
                    smmaCurve.Amount[i] = minDataList.get(i).get(0) * curVolume + smmaCurve
                            .Amount[i - 1];
                }
            }
...
//        Util.drawRect((int) startX, (int) startY, (int) (endX - startX + 2), (int) (endY -
//                startY), false, canvas, paint);
//        Util.drawRect((int) startX, (int) volumeStartY, (int) (endX - startX + 2), (int)
//                (volumeEndY - volumeStartY), false, canvas, paint);//量图边框
//        上边框
        Util.drawRect((int) startX, (int) startY, (int) endX, (int) (endY - startY), false,
                canvas, paint);
//        下边框
        Util.drawRect((int) startX, (int) volumeStartY, (int) endX, (int) (volumeEndY -
                volumeStartY), false, canvas, paint);//量图边框
...
//        float drawY = endY + FontHeight / 2 + 4;
        float drawY = volumeEndY + FontHeight / 2 + 4;

        if (mlineType == FIVE_DAY_MINUTES_LINE) {

            for (int i = 1; i < 5; i++) {
                float evenWidth = (endX - startX) / 5.0f;
                canvas.drawLine(startX + evenWidth * i, startY, startX + evenWidth * i, endY,
                        paint);
            }
            ...
            for (int i = 1; i < 8; i++) {
                float evenWidth = (endX - startX) / 8.0f;
                canvas.drawLine(startX + evenWidth * i, startY, startX + evenWidth * i, endY,
                        paint);
            }
//            绘制时间
        ...
        for (int i = 1; i < 8; i++) {
            float evenWidth = (endX - startX) / 8.0f;
            canvas.drawLine(startX + evenWidth * i, volumeStartY, startX + evenWidth * i,
                    volumeEndY, paint);
        }
        canvas.drawLine(startX, startY + (endY - startY) / 4, endX, startY + (endY - startY) / 4,
                paint);
        canvas.drawLine(startX, startY + (endY - startY) * 3 / 4, endX, startY + (endY - startY)
                * 3 / 4, paint);
        ...
        paint.setAntiAlias(true);
        canvas.drawLine(startX, startY + (endY - startY) * 2 / 4, endX + 1, startY + (endY -
                startY) * 2 / 4, paint);
    }
...
//                System.out.println("left: "+left);
                top = (float) ((endY - startY) / 2.0f + startY - (trendHeight) * (minDataList.get
                        (i).get(0) - closeTrend + 0.0f) / maxTrendDiff);
                hashMap.put(i, top);
...
//                System.out.println(" i: "+i+" lastX: "+lastX+" endX: "+endX+" offsetX:
// "+offsetX+" startX: "+startX+" offsetX: "+offsetX);
            }
...
//        }
        if (minDataList != null && minDataList.size() > 0) {
            ...
            Paint paint = new Paint();
            paint.setAntiAlias(true);
            LinearGradient gradient = new LinearGradient(2, endY, 2, 2, new int[]{Color.WHITE,
            ...
            for (int i = 0; i < minDataList.size() && i < MAX_MDATA_NUM; i++) {
                ...
                left += offsetX;
//                top = (float) ((endY - startY) / 2 + startY - (trendHeight) * (smmaCurve
// .SMMA[i] - closeTrend + 0.0f) / maxTrendDiff);
                top = (float) ((endY - startY) / 2 + startY - (trendHeight) * (minDataList.get(i)
                        .get(2) - closeTrend + 0.0f) / maxTrendDiff);
                if (top < startY) {
                    top = startY;
                }
                right = left + offsetX;
    ...
    public void paintKD(Canvas canvas) {
        ...
        kd1 = Util.saveNumFromat(String.valueOf((closeTrend + maxTrendDiff + 0.0f) + maxTrendDiff
                * 0.1), savepoint);
        kd2 = Util.saveNumFromat(String.valueOf((closeTrend + maxTrendDiff / 2 + 0.0f) +
                maxTrendDiff * 0.1), savepoint);
        kd3 = Util.saveNumFromat(String.valueOf((closeTrend + 0.0f)), savepoint);
        kd4 = Util.saveNumFromat(String.valueOf((closeTrend - maxTrendDiff / 2 + 0.0f) -
                maxTrendDiff * 0.1), savepoint);
        kd5 = Util.saveNumFromat(String.valueOf((closeTrend - maxTrendDiff + 0.0f) - maxTrendDiff
                * 0.1), savepoint);


        linePaint.setColor(CharColor);
//        左侧的数据
        canvas.drawText(kd1, startX, startY, linePaint);
        canvas.drawText(kd2, startX, startY + (endY - startY) / 4, linePaint);
        canvas.drawText(kd3, startX, startY + (endY - startY) * 2 / 4, linePaint);
        canvas.drawText(kd4, startX, startY + (endY - startY) * 3 / 4, linePaint);
        canvas.drawText(kd5, startX, endY, linePaint);
        ...
        if (closeTrend == 0) {
            ...
            if ((maxTrendDiff / closeTrend > 0 && maxTrendDiff / closeTrend < 0.00001) ||
                    (maxTrendDiff / closeTrend < 0 && maxTrendDiff / closeTrend > -0.00001)) {
                kd_1 = "0.00%";
                kd_2 = "0.00%";
                kd_3 = "0.00%";
                kd_4 = "0.00%";
        ...
        canvas.drawText(kd_3, endX - (Util.stringWidth(kd_3, linePaint) + 4), startY + (endY -
                startY) * 2 / 4, linePaint);
        linePaint.setColor(CharColor);
        canvas.drawText(kd_4, endX - (Util.stringWidth(kd_4, linePaint) + 4), startY + (endY -
                startY) * 3 / 4, linePaint);
        canvas.drawText(kd_5, endX - (Util.stringWidth(kd_5, linePaint) + 4), endY, linePaint);
//        }
    }
...
//            x = startX - Util.stringWidth(content[1], paint);
    ...
    public String getTime() {
        StringBuffer sb = new StringBuffer(String.valueOf(minDataList.get(getIndex()).get(2)
                .longValue()));
        if (sb.length() == 3) {
            sb.insert(0, "0");
        }
    ...
    public SpannableString getChangeRate() {
        SpannableString ss = new SpannableString("涨幅：" + DoubleUtil.formatDoubleDot2
                (getChangeRateValue()) + "%");
        if (getChangeRateValue() < 0) {
            ss.setSpan(new ForegroundColorSpan(getResources().getColor(android.R.color
                    .holo_green_dark)), 0, ss.length(), SpannableString.SPAN_INCLUSIVE_EXCLUSIVE);
    ...
    public String getAmount() {
        return "成交" + Util.translate_long_thousand((long) (getIndex() == 0 ? minDataList.get
                (getIndex()).get(1) / 100 : (minDataList.get(getIndex()).get(1) - minDataList.get
                (getIndex() - 1).get(1)) / 100)) + "手";
    }
